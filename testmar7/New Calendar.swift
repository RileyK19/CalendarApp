...
struct CalendarDayView: View {
    ...
    func overlap(x: Int) -> Int {
        let calendar = Calendar.current
        let first1 = calendar.date(byAdding: .minute, value: -20, to: events[x].start)!
        let first2 = calendar.date(byAdding: .minute, value: 20, to: events[x].start)!
        let second1 = calendar.date(byAdding: .minute, value: -20, to: events[x].end)!
        let second2 = calendar.date(byAdding: .minute, value: 20, to: events[x].end)!
        var ret = 0
        for y in 0..<x {
            if first1 <= events[y].start && second2 >= events[y].end {
                ret += 1
            } else if first2 >= events[y].start && second1 <= events[y].end {
                ret += 1
            } else if (first2 >= events[y].start) == (second1 <= events[y].end) {
                ret += 1
            } else if (first1 <= events[y].start) == (second2 >= events[y].end) {
                ret += 1
            }
        }
        return ret
...
struct CalendarDayViewEvent : View {
    ...
    func getTime() -> CGFloat {
        var min: Double = Double(Calendar.current.dateComponents([.minute], from: event.start).minute!) * 1.5
        var hr: Double = Double(Calendar.current.dateComponents([.hour], from: event.start).hour!) * 68 * 1.5
    ...
    func getInBetween() -> CGFloat {
        ...
        if !isSameDay(date1: day, date2: event.start) && day > event.start {
            first = day.startOfDay
        }
        if !isSameDay(date1: day, date2: event.end) && day < event.end {
            second = day.endOfDay
        }
        let min: Double = Double(calendar.dateComponents([.minute], from: second).minute! - calendar.dateComponents([.minute], from: first).minute!) * 1.5
        let hr: Double = Double(calendar.dateComponents([.hour], from: second).hour! - calendar.dateComponents([.hour], from: first).hour!) * 68 * 1.5
        return CGFloat(min + hr)
    }
